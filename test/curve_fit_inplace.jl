for T in (Float32, Float64, ComplexF32, ComplexF64)
    let
        ...
        # some example data
        Random.seed!(12345)
        xdata = range(zero(T), stop=10, length=500000)
        ydata = model(xdata, T[1.0, 2.0]) + 0.01*randn(length(xdata))
        p0 = T[0.5, 0.5]
        ...
        function jacobian_model_inplace(J::Array{T,2},x,p) where {T}
            @. J[:,1] = exp(-x*p[2])     #dmodel/dp[1]
            @. @views J[:,2] = -x*p[1]*J[:,1]
        end
        ...
        @test maximum(abs.(fit_wt.param - fit_inplace_wt.param)) < (T in [Float64, ComplexF64] ? 1e-15 : 1e-7)
        ...
        @test maximum(abs.(fit_wt_jac.param - fit_inplace_wt_jac.param)) < (T in [Float64, ComplexF64] ? 1e-15 : 1e-7)
